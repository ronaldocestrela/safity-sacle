...
public sealed record SectorDto(Guid Id, string Name, string? Description, bool IsActive, DateTime CreatedAt);

public static class SectorMappings
{
    public static SectorDto ToDto(this Sector sector)
        => new(sector.Id, sector.Name, sector.Description, sector.IsActive, sector.CreatedAt);
    ...
    public static SectorDto ToSummaryDto(this Sector sector)
        => new(sector.Id, sector.Name, null, sector.IsActive, sector.CreatedAt);
}
